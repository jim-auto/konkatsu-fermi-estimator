...
現在の先頭コミット: `f39a2f9 Refine hero copy`
...
Konkatsu Fermi Estimator は、婚活条件を指定すると「その条件に当てはまる異性がどれくらい存在するか」をフェルミ推定で見せる React アプリです。
...
- H1: `理想の相手、何人残る？`
- サブコピー: `日本人口を起点に、条件ごとの割合を掛け合わせるフェルミ推定です。条件を増やすほど、母集団がどれだけ削られるかを可視化します。`
...
- 上部ラベル: `Konkatsu Fermi Estimator`
- H1: `理想の相手、何人残る？`
- サブコピーでフェルミ推定を説明

H1 は体験寄りのコピーにして、フェルミ推定はサブコピーで説明する方針です。  
`フェルミ推定で理想の相手は何人いるのか` のような説明的な H1 は微妙という判断で変更済みです。
...
現在は Recharts の折れ線グラフに加えて、下に条件ごとのステップカードを表示しています。
...
この部分は直近で改善した重要箇所です。  
以前は `x0.184` と残存人数だけの小さいリストでしたが、ユーザーから「もっと見やすく。あとフェルミ推定も推して」と要望があり、現在のカード形式に変更しました。
...
条件同士の相関補正は未実装です。  
都市部と年収、年齢と未婚率、外見とスペ値などは本来独立ではありませんが、現状はUXと分かりやすさを優先して独立に近いものとして扱っています。
...
- `around20`: アラツー（18-24歳）
- `around30`: アラサー（25-34歳）
- `around40`: アラフォー（35-44歳）
- `around50`: アラフィフ前半（45-49歳）

デフォルトは `around20` です。  
ユーザー要望により、アラツーがデフォルトになっています。

年齢レンジの人口と未婚率:

| ID | 表示 | 男性人口 | 女性人口 | 男性未婚率 | 女性未婚率 |
| --- | --- | ---: | ---: | ---: | ---: |
| `around20` | アラツー（18-24歳） | 4,365,000 | 4,105,000 | 96.7% | 94.7% |
| `around30` | アラサー（25-34歳） | 6,644,000 | 6,270,000 | 63.8% | 51.8% |
| `around40` | アラフォー（35-44歳） | 7,379,000 | 7,122,000 | 35.2% | 23.6% |
| `around50` | アラフィフ前半（45-49歳） | 4,431,000 | 4,313,000 | 29.9% | 19.2% |
...
7. 年齢レンジをアラツー/アラサー/アラフォー/アラフィフ前半へ変更
8. BMI/体型をスペ値へ変更
9. カップ数を F/G/H以上まで拡張
...
- `推定式` を小さく表示する
- `日本の女性 → アラツー → 未婚 → 都市部 → ...` のようにパンくず型で見せる
